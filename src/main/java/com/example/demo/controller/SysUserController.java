...
@RequestMapping("/sysuser") // swagger访问地址 http://localhost:8080/swagger-ui.html

@Api("SysUserController相关的api")
public class SysUserController { // 调用同一个接口的不同实现类，实现类要定义@Service("name")
    ...
    @RequestMapping(value = "/listSysUser", method = RequestMethod.GET) // 这样pageSize就可以为空了，此时int型的pageNum如果没有传参怎么会报错
    public List<SysUser> listSysUser(int pageNum,
